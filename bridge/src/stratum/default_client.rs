...
static BIG_JOB_REGEX: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r".*(BzMiner|IceRiverMiner).*").unwrap());

/// Regex for matching wallet addresses
static WALLET_REGEX: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r"kaspa(test|dev)?:([a-z0-9]{61}|[a-z0-9]{63})").unwrap());
...
pub async fn handle_subscribe(
    ...
    tracing::debug!(
        "[SUBSCRIBE] ===== SUBSCRIBE REQUEST FROM {} =====",
        ctx.remote_addr
    );
    ...
    if let Some(Value::String(app)) = event.params.first() {
        ...
        tracing::warn!(
            "[SUBSCRIBE] No app string in params[0], params: {:?}",
            event.params
        );
    }
    ...
    tracing::debug!(
        "[SUBSCRIBE] Client info - app: '{}', extranonce: '{}', addr: {}",
        remote_app,
        extranonce,
        ctx.remote_addr
    );
    ...
    tracing::debug!(
        "[SUBSCRIBE] Detected miner type - Remote app: '{}', Is Bitmain: {}",
        remote_app,
        is_bitmain
    );
    ...
    if is_bitmain {
        ...
        tracing::debug!(
            "[SUBSCRIBE] Current extranonce: '{}' (length: {} bytes)",
            extranonce,
            extranonce.len() / 2
        );
    }
    ...
    let response = if is_bitmain {
        ...
        tracing::debug!(
            "[SUBSCRIBE] ===== USING BITMAIN SUBSCRIBE FORMAT FOR {} =====",
            ctx.remote_addr
        );
        tracing::debug!(
            "[SUBSCRIBE] Bitmain extranonce: '{}', extranonce2_size: {}",
            extranonce,
            extranonce2_size
        );
        tracing::debug!(
            "[SUBSCRIBE] Bitmain response: [null, '{}', {}]",
            extranonce,
            extranonce2_size
        );
        JsonRpcResponse::new(
        ...
        if BIG_JOB_REGEX.is_match(&remote_app) {
            tracing::debug!(
                "[SUBSCRIBE] Using standard subscribe format for IceRiver/BzMiner {}",
                ctx.remote_addr
            );
        } else {
            tracing::debug!(
                "[SUBSCRIBE] Using standard subscribe format for {}",
                ctx.remote_addr
            );
        }
    ...
    tracing::debug!(
        "[SUBSCRIBE] Sending subscribe response to {}: {}",
        ctx.remote_addr,
        response_json
    );
    ...
    ctx.reply(response)
        .await
        .map_err(|e| format!("failed to send response to subscribe: {}", e))?;

    tracing::debug!(
        "[SUBSCRIBE] ===== SUBSCRIBE COMPLETE FOR {} =====",
        ctx.remote_addr
    );
    Ok(())
}
...
async fn handle_extranonce_subscribe(
    ...
    tracing::debug!(
        "[EXTRANONCE_SUBSCRIBE] ===== EXTRANONCE SUBSCRIBE FROM {} =====",
        ctx.remote_addr
    );
    ...
    let response_json = serde_json::to_string(&response).unwrap_or_else(|_| "failed".to_string());
    tracing::debug!(
        "[EXTRANONCE_SUBSCRIBE] Sending response to {}: {}",
        ctx.remote_addr,
        response_json
    );
    ...
    ctx.reply(response)
        .await
        .map_err(|e| format!("failed to send response to extranonce subscribe: {}", e))?;

    tracing::debug!(
        "[EXTRANONCE_SUBSCRIBE] ===== EXTRANONCE SUBSCRIBE COMPLETE FOR {} =====",
        ctx.remote_addr
    );
    Ok(())
}
...
pub async fn handle_authorize(
    ...
    tracing::debug!(
        "[AUTHORIZE] ===== AUTHORIZE REQUEST FROM {} =====",
        ctx.remote_addr
    );
    ...
    let address_str = address_value
        .as_str()
        .ok_or("expected param[0] to be address string")?;

    tracing::debug!(
        "[AUTHORIZE] Address string from params[0]: '{}'",
        address_str
    );

    let parts: Vec<&str> = address_str.split('.').collect();
    tracing::debug!(
        "[AUTHORIZE] Split address into {} parts: {:?}",
        parts.len(),
        parts
    );
    ...
    if parts.len() >= 2 {
        ...
        if parts.len() >= 3 {
            ...
            tracing::debug!(
                "[AUTHORIZE] Extracted canxium address: '{}'",
                canxium_address
            );
        }
    }
    ...
    tracing::debug!(
        "[AUTHORIZE] Final parsed - address: '{}', worker: '{}', canxium: '{}'",
        address,
        worker_name,
        canxium_address
    );
    ...
    let response_json = serde_json::to_string(&response).unwrap_or_else(|_| "failed".to_string());
    tracing::debug!(
        "[AUTHORIZE] Sending authorize response to {}: {}",
        ctx.remote_addr,
        response_json
    );
    ...
    if !extranonce.is_empty() {
        tracing::debug!(
            "[AUTHORIZE] Step 2: Sending extranonce to client {} before difficulty/job",
            ctx.remote_addr
        );
        tracing::debug!("[AUTHORIZE] Extranonce value: '{}'", extranonce);
        send_extranonce(ctx.clone()).await?;
        tracing::debug!(
            "[AUTHORIZE] Extranonce sent successfully to client {}",
            ctx.remote_addr
        );
    } else {
        tracing::debug!(
            "[AUTHORIZE] No extranonce configured (extranonce_size=0), skipping extranonce step"
        );
    }
    ...
    if let (Some(client_handler), Some(kaspa_api)) = (client_handler, kaspa_api) {
        ...
        client_handler
            .send_immediate_job_to_client(ctx.clone(), kaspa_api)
            .await;
    } else {
    ...
    tracing::debug!(
        "[AUTHORIZE] ===== AUTHORIZE COMPLETE FOR {} =====",
        ctx.remote_addr
    );
    Ok(())
}
...
async fn send_extranonce(
    ...
    tracing::debug!(
        "[EXTRANONCE] ===== SENDING EXTRANONCE TO {} =====",
        ctx.remote_addr
    );
    ...
    tracing::debug!(
        "[EXTRANONCE] Remote app: '{}', Extranonce: '{}'",
        remote_app,
        extranonce
    );
    ...
    let is_bitmain = remote_app_lower.contains("godminer")
        ...
        || remote_app_lower.contains("antminer");
    tracing::debug!(
        "[EXTRANONCE] Detected miner type - Remote app: '{}', Is Bitmain: {}",
        remote_app,
        is_bitmain
    );
    ...
    let params = if is_bitmain {
        let extranonce2_size = 8 - (extranonce.len() / 2);
        tracing::debug!(
            "[EXTRANONCE] ===== USING BITMAIN EXTRANONCE FORMAT FOR {} =====",
            ctx.remote_addr
        );
        tracing::debug!(
    ...
    if is_iceriver {
        ...
        tracing::debug!(
            "[EXTRANONCE] Sending mining.set_extranonce to {}: {}",
            ctx.remote_addr,
            event_json
        );
        ctx.send(event)
